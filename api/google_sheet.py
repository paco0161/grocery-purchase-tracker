import os.path

from google.auth.transport.requests import Request
from google.oauth2.credentials import Credentials
from google_auth_oauthlib.flow import InstalledAppFlow
from googleapiclient.discovery import build
from googleapiclient.errors import HttpError

from models.sheet_request import SheetRequest

class SheetAPIClient:
    def __init__(self):
      self.scopes = ["https://www.googleapis.com/auth/spreadsheets"]
      self.creds = self.get_sheet_api_client()

    def get_sheet_api_client(self):
      """Shows basic usage of the Sheets API.
      Prints values from a sample spreadsheet.
      """
      creds = None
      # The file token.json stores the user's access and refresh tokens, and is
      # created automatically when the authorization flow completes for the first
      # time.
      if os.path.exists("token.json"):
        creds = Credentials.from_authorized_user_file("token.json", self.scopes)
      # If there are no (valid) credentials available, let the user log in.
      if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
          creds.refresh(Request())
        else:
          flow = InstalledAppFlow.from_client_secrets_file(
              "../credentials.json", self.scopes,
          )
          creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open("token.json", "w") as token:
          token.write(creds.to_json())

        return creds
      
    def build_service(self):
        return build("sheets", "v4", credentials=self.creds)
    
    def append(self, values: list=None, **kwargs):
        try:
            service = self.build_service()

            # Call the Sheets API
            sheet = service.spreadsheets().values()
            request = SheetRequest.SheetRequest(values=values)
            result = (
                sheet
                .append(spreadsheetId=kwargs['SPREADSHEET_ID'], range=kwargs['RANGE_NAME'],
                        includeValuesInResponse=kwargs['INCLUDE_VALUES_IN_RESPONSE'],
                        valueInputOption=kwargs['VALUE_INPUT_OPTION'], insertDataOption=kwargs['INSERT_DATA_OPTION'], body=request)
                .execute()
            )
            
            print(f'''Updated spreadsheet table {result['tableRange']}, 
                append data {result['updates']['updatedData']['values']}
                in {result['updates']['updatedRange']}''')

        except HttpError as err:
            print(err)
